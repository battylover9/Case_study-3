      ...
      environment = [
        {
          ...
          value = "http://grafana.${var.private_zone_name}:3000"
        },
        {
  ...
  content = <<-EOT
    ...
    datasources:
      - name: Prometheus
        type: prometheus
        access: proxy
        url: http://prometheus.${var.private_zone_name}:9090
        isDefault: true
        editable: false
      ...
      - name: Loki
        type: loki
        access: proxy
        url: http://loki.${var.private_zone_name}:3100
        editable: false
...
output "grafana_internal_url" {
  ...
  value       = "http://grafana.${var.private_zone_name}"
}
...
output "grafana_info" {
  ...
  - Internal URL: http://grafana.${var.private_zone_name}
